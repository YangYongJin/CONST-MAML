    ...
    def __init__(self, input_feature_shape, eps=1e-5, elementwise_affine=True):
        ...
        if self.elementwise_affine:
            self.weight = nn.Parameter(torch.Tensor(
                *input_feature_shape), requires_grad=False)
        ...
        if self.normalization:
            if self.args.norm_layer == "batch_norm":
                ...
                self.norm_layer = MetaLayerNormLayer(
                    input_feature_shape=out.shape[1:])
...
class MetaNormLayerConvReLU(nn.Module):
    ...
    def build_block(self):
        ...
        out = self.layer_dict['activation_function_pre'].forward(
    ...
    def build_network(self):
        ...
        for i in range(self.num_stages):
            ...
            out = self.layer_dict['conv{}'.format(i)](
                out, training=True, num_step=0)

            if self.args.max_pooling:
                out = F.max_pool2d(input=out, kernel_size=(
        ...
        self.layer_dict['decoder1'] = MetaLinearLayer(input_shape=(out.shape[0], np.prod(out.shape[1:])),
                                                      num_filters=np.prod(out.shape[1:]), use_bias=True)
        self.layer_dict['decoder2'] = MetaLinearLayer(input_shape=(out.shape[0], np.prod(out.shape[1:])),
                                                      num_filters=np.prod(out.shape[1:]), use_bias=True)

        out = self.layer_dict['linear'](out)
        print("VGGNetwork build", out.shape)

    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False, decoding=False):
        """
        ...
        if decoding:
            out = self.layer_dict['decoder1'](out, param_dict['decoder1'])
            out = self.relu(out)
            out = self.layer_dict['decoder2'](out, param_dict['decoder2'])
        else:
