...
class MAMLFewShotClassifier(nn.Module):
    ...
    def info_nce_loss(self, features):
        ...
        labels = torch.cat([torch.arange(features.shape[0]//2)
    ...
    def forward(self, data_batch, epoch, use_second_order, use_multi_step_loss_optimization, num_steps, training_phase):
        ...
        for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in enumerate(zip(x_support_set,
            ...
            x_unsup = [self.augmentation(x_target_set_task) for i in range(2)]
            x_unsup = torch.cat(x_unsup, dim=0).to(self.device)
            ...
            for num_step in range(num_steps):
                ...
                support_contrastive_loss, _ = self.net_forward(
                    x=x_unsup,
                    y=y_support_set_task,
                    weights=names_weights_copy,
                    backup_running_statistics=num_step == 0,
                ...
                support_loss += support_contrastive_loss
    ...
    def net_forward(self, x, y, weights, backup_running_statistics, training, num_step, decoding=False):
        """
        ...
        preds = self.classifier.forward(x=x, params=weights,
                                        ...
                                        decoding=decoding)
        if decoding:
            preds, labels = self.info_nce_loss(preds)
    ...
    def run_train_iter(self, data_batch, epoch):
        ...
        x_support_set = torch.Tensor(
            x_support_set).float().to(device=self.device)
        x_target_set = torch.Tensor(
            x_target_set).float().to(device=self.device)
        y_support_set = torch.Tensor(
            y_support_set).long().to(device=self.device)
